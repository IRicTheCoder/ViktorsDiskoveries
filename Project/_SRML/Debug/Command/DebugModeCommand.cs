...
namespace VikDisk.SRML.Debug
{
	public class DebugModeCommand : ConsoleCommand
	{
		...
		private void RunHelp()
		{
			...
			Console.LogSuccess("<color=white>debugmode state <value></color> - <value> can be true or false to activate or deactivate the debug mode", false);
			Console.LogSuccess("<color=white>debugmode markers <value> - <value></color> can be true or false to activate or deactivate the markers", false);
		}

		private void RunState(bool state)
		{
			DebugHandler.IsDebugging = state;
			Console.LogSuccess($"Turned the debug mode {(state ? "on" : "off")}");
		}

		private void RunMarkers(bool state)
		{
			MarkerController.ShowMarkers = state;
			Console.LogSuccess($"Turned the markers visibility {(state ? "on" : "off")}");
		}
